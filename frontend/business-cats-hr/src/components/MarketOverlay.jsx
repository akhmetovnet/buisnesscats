...
const buildTradePriceHint = (item, parsedPrice) => {
  const referencePrice = Number(item?.referencePrice ?? 0)
  if (!Number.isFinite(parsedPrice) || parsedPrice <= 0 || referencePrice <= 0) return null
...
export default function MarketOverlay({
  ...
  const validateSellPayload = (payload) => {
    ...
    const kittenStatus = resolveKittenStatus(payload, adultAge)
  ...
  const executeTrade = async () => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('not found') || msg.includes('trade_requests_unavailable')) {
        ...
        setTradeValidationError(formatTradeRequestError(msg) || 'Не удалось отправить заявку')
      }
    }
  }
              ...
              <div className="lot-area__lot-main">
                ...
                <div className="trade-items">
                  {tradeItems.map((item) => {
                    ...
                    const priceHint = buildTradePriceHint(item, parsedPrice)
                    ...
                    return (
                      <div className="trade-item" key={item.key}>
                        ...
                        <div className="trade-item__meta">
                          ...
                          {item.referencePrice ? (
                            <div className="trade-item__market-note">
                              Цена магазина: <span className="notranslate">{item.referencePrice}</span>
                            </div>
                          ) : null}
                          {priceHint ? (
                            <div className={`trade-item__hint trade-item__hint--${priceHint.tone}`}>
                              {priceHint.label}
                            </div>
                          ) : null}

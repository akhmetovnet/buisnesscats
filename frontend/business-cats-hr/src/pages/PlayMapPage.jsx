...
const describeEscapedHungryCat = (cat) => {
  ...
  const role = cat?.isKitten ? 'котенок' : sex === 'F' ? 'кошка' : 'кот'
...
const buildSeasonTransition = (prevNursery) => {
  ...
  const escapedHungryById = new Map()
  const markEscapedHungry = (cat) => {
    if (!cat || !cat.id || !cat.hungry) return
    const id = String(cat.id)
    if (escapedHungryById.has(id)) return
    escapedHungryById.set(id, { ...cat, id })
  }

  const prevCats = Array.isArray(prev?.cats) ? prev.cats : []
  const prevKittens = Array.isArray(prev?.home?.kittens) ? prev.home.kittens : []
  prevCats.forEach(markEscapedHungry)
  prevKittens.forEach(markEscapedHungry)

  const escapedHungryIds = new Set(escapedHungryById.keys())
  const escapedCatIdsSet = new Set([...persistedEscapedIds, ...escapedHungryIds])
  const escapedCatIds = Array.from(escapedCatIdsSet)
  const escapedHungryCats = Array.from(escapedHungryById.values())
  ...
  cats = cats.map((cat) => {
    ...
    const staysKitten = Boolean(cat.isKitten) && nextAge < 3
    return {
      ...cat,
      age: nextAge,
...
const buildCombinedSeasonResult = ({
  ...
  return {
    ...
    coinsEnd,
    salesProfit,
    escapedCats: Math.max(0, Number(escapedCats) || 0),
    expenses,
    profit,
    creditDelta,
    expenseBreakdown: {
...
const normalizeNurseryCat = (cat, { forceKitten = null } = {}) => {
...
const normalizeNurseryState = (rawNursery) => {
  ...
  const cats = (Array.isArray(rawNursery.cats) ? rawNursery.cats : [])
    .map((cat) => normalizeNurseryCat(cat))
    .filter((cat) => !escapedCatIdsSet.has(String(cat?.id ?? '')))
    .filter(Boolean)
  ...
  const kittens = Array.from({ length: 12 }, (_, idx) => {
    const kitten = normalizeNurseryCat(sourceKittens[idx], { forceKitten: true })
    if (!kitten) return null
...
export default function PlayMapPage({ me }) {
  ...
  const inventoryForUi = useMemo(() => {
    ...
    backendInventoryEntities.forEach((entity) => {
      ...
      const isKitten = entity?.isKitten ?? age < 3
      if (!isKitten) return
    ...
    return next
  }, [inventory, backendInventoryEntities, escapedCatIdsSet])
  ...
  const normalizedBackendKittens = useMemo(
    () =>
      backendInventoryEntities
        ...
        .filter(Boolean),
    [backendInventoryEntities, escapedCatIdsSet]
  )

  useEffect(() => {
    if (!normalizedBackendKittens.length) return
    setNursery((prev) => {
      ...
      const existingIds = new Set(
        [...(prev?.cats || []), ...((prev?.home?.kittens || []).filter(Boolean))]
          .map((cat) => cat?.id)
          .filter(Boolean)
      )
      const missing = normalizedBackendKittens.filter(
        (cat) => cat?.id && !escapedIds.has(cat.id) && !existingIds.has(cat.id)
      )
      if (!missing.length) return prev
      return { ...prev, cats: [...(prev.cats || []), ...missing] }
    })
  }, [normalizedBackendKittens])
  ...
  const inventoryEntities = useMemo(() => {
    ...
    return merged
  }, [backendInventoryEntities, nursery?.cats, nursery?.home?.kittens, playerRole, escapedCatIdsSet])
  ...
  }, [sessionId, season, resolveCounterpartyContext, selectedBuilding?.id, selectedBuilding?.type, handleInactivityTimeoutError])
  ...
  }, [sessionId, season, activeRequest?.id, handleInactivityTimeoutError])
  ...
  useEffect(() => {
    ...
    const applyProgress = (parsed) => {
      ...
      setNursery(normalizeNurseryState(parsed.nursery))
  ...
  }, [season, sessionId, progressStorageKey, seasonDuration])
  ...
  const handleCreateTradeRequest = useCallback(
    ...
    [sessionId, season, loadTradeRequests, loadData]
  )
  ...
  const handleRequestAction = useCallback(
    async (action, extra = {}) => {
      ...
      try {
        ...
        setError(err.message || 'trade_request_action_failed')
      } finally {
        setTradeRequestsBusy(false)
      }
    },
  ...
  const handleEndSeason = useCallback(async ({ auto = false, finishEarly = false } = {}) => {
    ...
    try {
      ...
      const seasonTransition = buildSeasonTransition(nurseryRef.current)
      setNursery(seasonTransition.nursery)
      setEscapedHungryCats(seasonTransition.escapedHungryCats)
        ...
        res
          ? {
              ...
              seasonResult: buildCombinedSeasonResult({
                ...
                escapedCats: seasonTransition?.escapedHungryCats?.length ?? 0,
              }),
            }
          : null
      )
  ...
  }, [season, sessionId, seasonFinishedLocked, seasonLedger])
  ...
  const persistProgressForSeason = useCallback(
    async (targetSeason) => {
      ...
      const nextStartCoins = normalizeNurseryState(nursery).coins
      const nextPayload = {
        nursery: normalizeNurseryState(nursery),
    ...
    [sessionId, nursery, seasonLedger, season, coinsNow, nurseryCoinsDelta, SEASON_SECONDS]
  )
  ...
  const continueAfterSeasonResult = useCallback(
    ...
    [persistProgressForSeason, navigate, sessionId]
  )
        ...
        <div className="modal-overlay" onClick={handleEscapedHungryConfirm}>
          <div className="modal modal--size-big escaped-hungry-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal__header">
              <div className="modal__title">СБЕЖАЛИ ГОЛОДНЫЕ КОТИКИ</div>
              <div className="modal__desc">эти котики сбежали, потому что вы не покормили их в сезоне</div>
            </div>
